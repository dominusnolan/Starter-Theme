<?php
	 ...
	 */
	 function starter_carousel_scripts() {
		wp_enqueue_script( 'jquery' );
	
		wp_enqueue_style( 'carousel-style', get_template_directory_uri() . '/css/jcarousel.css' );
	
		wp_enqueue_script( 'carousel-script', get_template_directory_uri() .  '/js/jcarousel.js', array( 'jquery' ), false, true );
		
	}
	add_action( 'wp_enqueue_scripts', 'starter_carousel_scripts' );
		...
		if ( strpos( $source, 'post' ) !== false ) {
			...
			foreach ( $attachments as $attachment ) {
				...
				$return[] = array(
					...
					'thumbnail' => get_template_directory_uri() . '/inc/timthumb.php?src=' . $attachment->guid . '&amp;w=' . $width . '&amp;h=' . $height . '&amp;zc=1&amp;q=100',
					'link' => $linked,
		...
		elseif ( strpos( $source, 'cat=' ) !== false ) {
			...
			foreach ( $images as $image ) {
				...
				$return[] = array(
					...
					'thumbnail' => get_template_directory_uri() . '/inc/timthumb.php?src=' . su_get_post_image( $image->ID ) . '&amp;w=' . $width . '&amp;h=' . $height . '&amp;zc=1&amp;q=100',
					'link' => $linked,
